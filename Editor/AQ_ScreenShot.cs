...
[ExecuteInEditMode]
public class AQ_ScreenShot : EditorWindow
{
	...
	[MenuItem("Tools/AQ/High-Res Screenshot")]
	public static void ShowWindow()
	{
		//Show existing window instance. If one doesn't exist, make one.
		EditorWindow editorWindow = EditorWindow.GetWindow(typeof(AQ_ScreenShot));
	...
	void OnGUI()
	{
		EditorGUILayout.LabelField("ScreenShot Taker AQ", EditorStyles.centeredGreyMiniLabel);
		...
		if (GUILayout.Button("Take Screenshot", GUILayout.MinHeight(60)))
		{
			if (myCamera == null) 
			{
				Debug.LogError("Select Camera first.");return;
			}
			if (path == "")
			{
				path = EditorUtility.SaveFolderPanel("Path to Save Images", path, Application.dataPath);
